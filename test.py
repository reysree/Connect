...
def generate_graphs(dining_area):
    # Load Data (Please replace the path with the actual path to your CSV file)
    data = pd.read_csv('Food_Wastage_sample.csv')
    
    # Filter data for the specified dining area
    # Filter data based on user input
    filtered_data = data[data['Location'] == dining_area]
    filtered_data.head()
    ...
    plt.title('Highest Selling Food Item per Week in ' + dining_area)
    plt.xlabel('Week')
    ...
    plt.title('Weekly Food Wastage in ' + dining_area)
    plt.xlabel('Week')
    ...
    highest_selling_food_per_week = pd.DataFrame(columns=['Week', 'Food_Item', 'Amt_Consumed'])
    ...
    plt.title('Highest Selling Food Item per Week in ' + dining_area + ' (with Prediction)')
    plt.xlabel('Week')
    plt.ylabel('Amount Consumed')
    plt.xticks(highest_selling_food_per_week['Week'])
    plt.legend()
    ...
    plt.title('Weekly Food Wastage in ' + dining_area + ' (with Prediction)')
    plt.xlabel('Week')
    plt.ylabel('Total Wastage')
    plt.xticks(weekly_wastage['Week'])
    plt.grid(True)
